...
type UserRecordResult =
  | { totalAmount?: number; daysWithRecords?: number; error?: string }
  | { record?: number; daysWithRecords?: number; error?: string };
...
const ExpenseStats = async () => {
  try {
    const [userRecordResult, rangeResult] = await Promise.all([
      getUserRecord() as Promise<UserRecordResult>,
      getBestWorstExpense() as Promise<{ bestExpense?: number; worstExpense?: number }>,
    ]);

    // Backward-compatible extraction
    const totalAmount =
      ('totalAmount' in userRecordResult && typeof userRecordResult.totalAmount === 'number'
        ? userRecordResult.totalAmount
        : 0) ||
      ('record' in userRecordResult && typeof userRecordResult.record === 'number'
        ? userRecordResult.record
        : 0);

    const daysWithRecords =
      typeof userRecordResult.daysWithRecords === 'number'
        ? userRecordResult.daysWithRecords
        : 0;

    const safeDays = daysWithRecords > 0 ? daysWithRecords : 1;
    const averageExpense = totalAmount / safeDays;
          ...
          <div className="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-600 rounded-xl p-3 sm:p-4 border border-gray-200/50 dark:border-gray-600/50">
            <div className="text-center">
              ...
              <div className="inline-flex items-center gap-2 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 px-2 py-1 rounded-full text-xs font-medium">
                ...
                Based on {safeDays} day{safeDays === 1 ? '' : 's'} with expenses
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-lg font-bold text-red-600 dark:text-red-300">
                    {typeof bestExpense === 'number' ? currency.format(bestExpense) : 'No data'}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-lg font-bold text-green-600 dark:text-green-300">
                    {typeof worstExpense === 'number' ? currency.format(worstExpense) : 'No data'}
                  </p>
                </div>
              </div>
            </div>
    ...
    return (
      <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm p-8 rounded-2xl shadow-xl border border-gray-100/50 dark:border-gray-700/50 hover:shadow-2xl">
        ...
        <div className="bg-red-50/80 dark:bg-red-900/20 backdrop-blur-sm p-6 rounded-xl border-l-4 border-l-red-500">
          ...
          <p className="text-red-700 dark:text-red-400 text-sm ml-11">Please try again later</p>
        </div>
      </div>
    );
  }
